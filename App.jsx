...
export default function App() {
  ...
  const [logs,    setLogs]    = useState([
    "System ready.",
    "Click 'Choose File' below to load a video.",
  ]);
  ...
  const loadFile = useCallback((file) => {
    ...
    vid.onloadedmetadata = () => {
      ...
      log(`✅ Loaded — ${w}×${h}, ${fmtTime(vid.duration)}`);
    };
    vid.onerror = () => log("❌ Could not load this video format.");
  }, [log]);
  ...
  const start = () => {
    ...
    setPlaying(true);
    setRunning(true);
    log(`🔵 Hologram ON — mode: ${mode.toUpperCase()}`);
    animRef.current = requestAnimationFrame(renderLoop);
  };
  ...
  const reset = () => {
    ...
    log("🔄 System reset. Load a new video.");
  };
  ...
  const sec  = { fontSize:8, letterSpacing:3, color:"rgba(0,245,255,0.4)", textTransform:"uppercase",
                 fontFamily:"monospace", borderBottom:"1px solid rgba(0,245,255,0.1)", paddingBottom:6, marginBottom:12 };
  const bigBtn = (danger) => ({
    width:"100%", padding:"13px 0", fontFamily:"monospace", fontWeight:700,
    fontSize:11, letterSpacing:3, textTransform:"uppercase", cursor:"pointer", borderRadius:3,
    border:`1px solid ${danger?"#ff3366":C}`,
    background: danger?"rgba(255,51,102,0.1)":"rgba(0,245,255,0.08)",
    color: danger?"#ff3366":C,
  });
  const smBtn = {
    flex:1, padding:"8px 0", background:"transparent",
    border:"1px solid rgba(0,245,255,0.18)", color:"rgba(0,245,255,0.55)",
    fontSize:14, cursor:"pointer", borderRadius:2, fontFamily:"monospace",
  };

  return (
    <div style={{ background:"#000508", color:C, fontFamily:"'Share Tech Mono',monospace",
      ...
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        input[type=file] {
          color:#00ffd5; background:rgba(0,255,213,0.05);
          border:1px solid rgba(0,245,255,0.35); padding:9px 10px;
        ...
        input[type=file]::file-selector-button:hover { background:rgba(0,245,255,0.25); }
      ...
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between",
                    marginBottom:14, paddingBottom:12, borderBottom:"1px solid rgba(0,245,255,0.18)" }}>
        <span style={{ fontFamily:"'Orbitron',monospace", fontWeight:900, fontSize:22,
                       letterSpacing:5, textShadow:`0 0 24px ${C},0 0 50px rgba(0,245,255,0.2)` }}>
        ...
        <div style={{display:"flex", gap:22}}>
          {[["PARTICLES", pCount.toLocaleString()], ["FPS", fps], ["STATUS", running?"ACTIVE":"STANDBY"]].map(([l,v])=>(
            <div key={l} style={{textAlign:"right"}}>
              ...
              <div style={{fontSize:13, color:running?T:C, textShadow:`0 0 8px ${running?T:C}`}}>{v}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{display:"grid", gridTemplateColumns:"210px 1fr 180px", gap:12, alignItems:"start"}}>
        ...
        <div style={{display:"flex", flexDirection:"column", gap:10}}>
          ...
          <div style={box}>
            ...
            {fname && <div style={{marginTop:8, fontSize:9, color:T, letterSpacing:1}}>✓ {fname}</div>}
            <div style={{marginTop:6, fontSize:8, color:"rgba(0,245,255,0.25)", letterSpacing:1}}>
              MP4 · MOV · WEBM · AVI
            </div>
          </div>
            ...
            {[
              ...
              <div key={l} style={{marginBottom:11}}>
                <div style={{display:"flex", justifyContent:"space-between", marginBottom:3}}>
                  <span style={{fontSize:8, color:"rgba(0,245,255,0.5)", letterSpacing:1.5, textTransform:"uppercase"}}>{l}</span>
          ...
          {/* Mode */}
          <div style={box}>
            <div style={sec}>③ Hologram Mode</div>
            <div style={{display:"grid", gridTemplateColumns:"1fr 1fr", gap:5}}>
              {MODES.map(m=>(
                <button key={m.id} onClick={()=>{ setMode(m.id); log(`Mode → ${m.label}`); }}
                  style={{ padding:"8px 0", fontFamily:"monospace", fontSize:9, letterSpacing:1,
          ...
          <div style={box}>
            ...
            <button style={{...bigBtn(running), opacity:loaded?1:0.3, cursor:loaded?"pointer":"not-allowed"}}
              disabled={!loaded} onClick={running?stop:start}>
              {running ? "■ STOP HOLOGRAM" : "▶ START HOLOGRAM"}
            </button>
            <button onClick={reset}
              style={{marginTop:8, width:"100%", padding:"8px 0", background:"transparent",
                      border:"1px solid rgba(0,245,255,0.15)", color:"rgba(0,245,255,0.35)",
        ...
        <div style={{display:"flex", alignItems:"center", justifyContent:"center"}}>
          <div style={{position:"relative", display:"inline-block"}}>
            ...
            <canvas ref={canvasRef} width={620} height={349}
              style={{display:"block", background:"#000",
                      boxShadow:`0 0 50px rgba(0,245,255,0.12), 0 0 100px rgba(0,245,255,0.05)`,
                      maxWidth:"100%"}} />
            {!loaded && (
              <div style={{position:"absolute", inset:0, display:"flex", flexDirection:"column",
                           alignItems:"center", justifyContent:"center", gap:10, pointerEvents:"none"}}>
                <div style={{fontSize:48, opacity:0.1}}>⬡</div>
                <div style={{fontSize:10, letterSpacing:4, color:"rgba(0,245,255,0.12)",
                             fontFamily:"monospace", textTransform:"uppercase"}}>No Signal</div>
                <div style={{fontSize:9, color:"rgba(0,245,255,0.08)", letterSpacing:2}}>
        ...
        <div style={{display:"flex", flexDirection:"column", gap:10}}>
          ...
          <div style={box}>
            ...
            {[["File",fname||"—"],["Resolution",res],["Duration",dur],["Time",curTime]].map(([l,v])=>(
              <div key={l} style={{padding:"5px 8px", borderLeft:"2px solid rgba(0,245,255,0.12)",
                                   marginBottom:7, background:"rgba(0,245,255,0.015)"}}>
                <div style={{fontSize:7, color:"rgba(0,245,255,0.28)", letterSpacing:2, textTransform:"uppercase"}}>{l}</div>
                <div style={{fontSize:11, color:T, wordBreak:"break-all", lineHeight:1.4}}>{v}</div>
              </div>
            ))}
          </div>
          ...
          <div style={box}>
            ...
            {/* Progress bar */}
            <div style={{height:4, background:"rgba(0,245,255,0.1)", borderRadius:2,
                         cursor:"pointer", position:"relative"}} onClick={seek}>
              <div style={{height:"100%", width:`${prog}%`,
                           background:`linear-gradient(90deg,${C},${T})`,
                           borderRadius:2, boxShadow:`0 0 6px ${C}`, transition:"width 0.1s"}} />
            </div>
          </div>

          {/* Log */}
          <div style={{...box, flex:1}}>
            <div style={sec}>System Log</div>
            <div style={{overflowY:"auto", maxHeight:220, display:"flex", flexDirection:"column", gap:3}}>
              {logs.map((l,i)=>(
                <div key={i} style={{fontSize:9, letterSpacing:0.5, lineHeight:1.6,
                  color: l.startsWith("✅")?"#00ffd5" : l.startsWith("❌")?"#ff3366" :
                         l.startsWith("🔵")?"#00f5ff" : "rgba(0,245,255,0.45)"}}>
                  {l}
                </div>
              ))}
            </div>
      ...
      {/* Footer */}
      <div style={{marginTop:14, paddingTop:10, borderTop:"1px solid rgba(0,245,255,0.1)",
                   display:"flex", justifyContent:"space-between", fontSize:8,
                   color:"rgba(0,245,255,0.2)", letterSpacing:2}}>
        <span>HOLO·MATRIX v3.0 — PARTICLE ENGINE</span>
        <span style={{color:running?"#00ffd5":"rgba(0,245,255,0.2)"}}>
          {running?"● HOLOGRAM ACTIVE":"○ STANDBY"}
        </span>
        <span>© 2025 HOLOMATRIX</span>
      </div>
    </div>
  );
}
